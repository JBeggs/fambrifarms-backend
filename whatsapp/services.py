    ...
    try:
        ...
        for item_data in parsed_items:
            ...
            if suggestions:
                ...
                # Extract base product name from search term (similar to kg product logic)
                base_product_name = parsed_message.product_name.lower()
                # Remove quantities
                base_product_name = re.sub(r'\d+', '', base_product_name).strip()
                # Remove packaging words
                ...
                search_words = temp_matcher._extract_search_words(parsed_message.product_name)
                
                if base_product_name or search_words:
                    # Find products that match the search term and have stock but aren't in suggestions
                    matching_products_with_stock = []
                    
                    # Query products matching the base name or search words

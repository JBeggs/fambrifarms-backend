...
class WhatsAppMessage(models.Model):
    ...
    def _extract_company_from_context(self):
        """Look at previous messages for company context - prioritize closest messages"""
        from datetime import timedelta
        
        # First, try to find company in the immediately preceding messages (same timestamp or very close)
        immediate_window = self.timestamp - timedelta(seconds=30)  # 30 second window for immediate context
        
        immediate_messages = WhatsAppMessage.objects.filter(
            chat_name=self.chat_name,
            timestamp__gte=immediate_window,
            timestamp__lte=self.timestamp,  # Up to current message timestamp
            is_deleted=False
        ).exclude(
            id=self.id  # Exclude current message
        ).order_by('-timestamp', '-id')[:5]  # Check last 5 messages in time order
        
        # Look for company in immediate context first
        for prev_msg in immediate_messages:
            company = django_message_parser.to_canonical_company(prev_msg.content)
            if company:
                return company
        
        # If no company found in immediate context, expand to 5-minute window
        extended_window = self.timestamp - timedelta(minutes=5)
        
        extended_messages = WhatsAppMessage.objects.filter(
            chat_name=self.chat_name,
            timestamp__gte=extended_window,
            timestamp__lt=immediate_window,  # Don't re-check immediate messages
            is_deleted=False
        ).exclude(
            id=self.id
        ).order_by('-timestamp', '-id')[:10]
        
        for prev_msg in extended_messages:
            company = django_message_parser.to_canonical_company(prev_msg.content)
            if company:
                return company
